...
const getCars = async function () {
	const _numberOfCarsAvailable = await contract.methods
		.viewNumberOfCarsAvailable()
		.call();
	const _cars = [];
...
document.querySelector("#carmarket").addEventListener("click", async (e) => {
	if (e.target.className.includes("buyBtn")) {
		...
		try {
			const result = await contract.methods
				...
				.send({ from: kit.defaultAccount });
			notification(
				`🎉 You successfully bought "${cars[index].name}". 🎉`
			);
			getCars();
			getBalance();
		} catch (error) {
	...
	if (e.target.className.includes("upgradeBtn")) {
		const index = e.target.id;
		let price = prompt(
			`Enter new price for "${cars[index].name} (cUSD) ":`
		);
		...
		if (price != null) {
			...
			notification(
				`🎉 You successfully upgraded "${cars[index].name}". 🎉`
			);
			getCars();
			getBalance();
		} else {
	...
	if (e.target.className.includes("cancelSaleBtn")) {
		...
		try {
			const result = await contract.methods
				...
				.send({ from: kit.defaultAccount });
			notification(
				`🎉 Cancelling sale of "${cars[index].name}" successful. 🎉`
			);
			getCars();
		} catch (error) {
			notification(`⚠️ ${error}.`);
